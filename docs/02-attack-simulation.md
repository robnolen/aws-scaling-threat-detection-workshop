...
If this fails with the error message **“Your requested instance type (t2.micro) is not supported in your requested Availability Zone (us-west-2d)…”** and you are using an account provided by AWS during an event, please delete the failed CloudFormation template, then delete the us-west-2d subnet, go to [VPC](https://us-west-2.console.aws.amazon.com/vpc/home),  select Subnets, check the box of subnet us-west-2d, select Actions from the top menu and Delete subnet (subnets with resources in them cannot be deleted).
